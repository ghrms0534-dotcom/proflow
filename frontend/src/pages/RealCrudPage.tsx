...
export function RealCrudPage({ resource }: { resource: RealResource }) {
  ...
  const agentType = ({ requirements: 'requirement', schedules: 'schedule', wbs: 'wbs', uiDesigns: 'ui_design', databaseDesigns: 'database_design', apiDesigns: 'api_design', developmentTasks: 'development', codeReviews: 'code_review', unitTests: 'unit_test', integrationTests: 'integration_test', qualityResults: 'quality', defects: 'defect', documents: 'document', outputs: 'delivery_output' } as Partial<Record<RealResource, AgentType>>)[resource];
  const deliveryAgent = agentType && ['quality', 'defect', 'document', 'delivery_output'].includes(agentType);
