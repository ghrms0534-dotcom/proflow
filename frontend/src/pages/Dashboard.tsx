...
export default function Dashboard({ onLogout }: DashboardProps) {
  ...
  const displayedSectionAgents = sectionAgents.map((agent) => {
    ...
    if (agent.section === '검증 · 품질' && dashboardData) return { ...agent, progress: dashboardData.deliveryAgent.progress, status: dashboardData.deliveryAgent.completedCount === 4 ? '완료' : '검증중', recentTasks: dashboardData.deliveryAgent.completedCount, riskLevel: dashboardData.deliveryAgent.hasFailure ? 'CRITICAL' as const : dashboardData.deliveryAgent.completedCount === 4 ? 'SAFE' as const : 'WARN' as const };
    ...
    if (agent.agentName === 'System Control Agent' && dashboardData) return { ...agent, progress: dashboardData.systemAgent.progress, status: dashboardData.systemAgent.completedCount === 3 ? '완료' : '점검중', recentTasks: dashboardData.systemAgent.completedCount, riskLevel: dashboardData.systemAgent.hasFailure ? 'CRITICAL' as const : dashboardData.systemAgent.completedCount === 3 ? 'SAFE' as const : 'WARN' as const };
